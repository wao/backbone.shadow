...
const { Debug, yaClass } = require( "ya-utils" );
...
yaClass( ShadowModel ).inherit( Backbone.Model ).extend( forwardMethods, forbiddenMethods, {
    initialize : function( pBackboneModel, options ){
        ...
        this.mContainer = null; // this is for custom container
        this.collection = null; // this is used by Backbone.Collection.
        this.mIdEventName = "change:" + this.mModel.idAttribute;
        if( _.has( options, "container" ) ){
            Debug.assert( "trigger" in options.container );
            Debug.assert( _.isFunction( options.container.trigger ) );
            this.mContainer = options.container;
        }
